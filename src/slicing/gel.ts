...
import { center, MAX_PIECES, type Piece } from './model';
...
const deformation = `
  vec3 deformGel(vec3 p, vec4 pose) {
    ...
    p.xz += gelKnife.xy * distance * indent * .3;
    return p;
  }
`;
...
export class BatchedGel {
  ...
  constructor(readonly height: number, mint: boolean) {
    ...
    this.material = new THREE.MeshPhysicalMaterial({
      color: mint ? '#e4fff3' : '#ffe9ee', roughness: .045, metalness: 0,
      transmission: .96, ior: 1.46, thickness: height, attenuationColor: new THREE.Color(mint ? '#239978' : '#d21d52'),
      attenuationDistance: 2.65, clearcoat: 1, clearcoatRoughness: .035, envMapIntensity: .92,
    });
    this.material.onBeforeCompile = shader => {
      Object.assign(shader.uniforms, { gelPoses: { value: this.poses }, gelBounds: { value: this.bounds }, gelHeight: { value: height }, gelKnife: { value: this.knife }, gelBack: { value: this.backTarget.texture }, gelBackSize: { value: this.backSize } });
      shader.vertexShader = `attribute float gelPart; uniform vec4 gelPoses[48]; uniform float gelHeight; uniform vec4 gelKnife;
        varying float vGelPart; varying vec3 vGelLocal;\n${deformation}` + shader.vertexShader;
      shader.vertexShader = shader.vertexShader.replace('#include <beginnormal_vertex>', `#include <beginnormal_vertex>
        vec4 gelPose = gelPoses[int(gelPart + 0.5)];
        objectNormal.xz /= 1.0 + gelPose.w * .35;
        objectNormal.y /= 1.0 - gelPose.w * .7;`);
      shader.vertexShader = shader.vertexShader.replace('#include <begin_vertex>', `#include <begin_vertex>
        vGelPart = gelPart; vGelLocal = position;
        transformed = deformGel(transformed, gelPose);`);
      shader.fragmentShader = `uniform vec4 gelBounds[48]; uniform float gelHeight; uniform sampler2D gelBack; uniform vec2 gelBackSize;
        varying float vGelPart; varying vec3 vGelLocal;
        float gelDepth(vec3 direction, vec3 viewPosition) {
          ...
          vec3 lower = vec3(b.x, .015, b.y), upper = vec3(b.z, gelHeight + .015, b.w);
    ...
    this.material.customProgramCacheKey = () => 'batched-jelly-measured-depth-v2';
    ...
    this.backMaterial = new THREE.ShaderMaterial({ side: THREE.BackSide, toneMapped: false,
      uniforms: { gelPoses: { value: this.poses }, gelHeight: { value: height }, gelKnife: { value: this.knife } },
      vertexShader: `attribute float gelPart; uniform vec4 gelPoses[48]; uniform float gelHeight; uniform vec4 gelKnife; varying vec3 backWorld; varying float backPart; ${deformation}
  ...
  rebuild(pieces: readonly Piece[]) {
    ...
    pieces.forEach((piece, index) => {
      ...
      const geometry = createGelSurface(polygon, origin, this.height);
  ...
  update(pieces: readonly Piece[], reduced: boolean) {
    pieces.forEach((piece, index) => {
      const wobble = reduced || piece.age > 1.8 ? 0 : Math.sin(piece.age * 15) * Math.exp(-piece.age * 4.4) * .047;
      this.poses[index].set(this.origins[index].x + piece.offset.x, this.origins[index].z + piece.offset.z, 0, wobble);
    });
  }
